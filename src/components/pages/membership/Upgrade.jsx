...
const Upgrade = observer(() => {
  ...
  const handleUpgradeUsers = async () => {
    if (membershipStore.selectedUsers.length === 0) {
      membershipStore.openErrorDialog(t('membership.upgrade.errors.noUsersSelected'));
      return;
    }
  ...
  return (
    <Page store={membershipStore} title={t('membership.upgrade.title')}>
      ...
      <h2 className="text-xl font-bold mb-6 pt-4 flex items-center gap-2 text-gray-900">
