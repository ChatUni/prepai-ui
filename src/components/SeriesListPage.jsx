...
const SeriesListPage = observer(() => {
  ...
  return (
    <div className="flex-1 p-3 pb-20 sm:p-4 md:p-6 md:pb-6 overflow-y-auto">
      {/* Carousel - only show in non-select mode */}
      {!isSelectMode && <Carousel images={coursesStore.seriesCovers} />}

      {/* Tools Navigation - only show in non-select mode */}
      {!isSelectMode && <ToolsNav />}
